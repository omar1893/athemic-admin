<template>
    <div class="max-w-2xl mx-auto py-8">
        ...
        <div class="items-center mb-5 flex justify-between">
            <div class="monserrat text-2xl font-semibold">{{ isEditing ? route.params.id : 'Nueva tienda' }}</div>
            <div class="monserrat">

                <button v-show="isEdited" @click="buttonCancel()"
                    class="m-5 font-semibold text-[#170033] text-sm">cancelar</button>

                <button type="submit" @click="buttonCondition()" class="px-4 py-3 rounded-full text-white text-sm font-semibold bg-[#875EF8] disabled:bg-[#875EF859]
            disabled:text-white disabled:cursor-not-allowed">
                    {{ isEditing ? editCondition : 'Crear Tienda' }}
                </button>
                    ...
                    <ModalDefault :show="saveProduct" @close="saveProduct = false">
                            ...
                            <div class=" monserrat flex mt-10 flex-col items-center justify-center">
                                <div class=" flex items-center justify-center w-12 h-12 rounded-full bg-[#875EF8]">
                                    <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                                        ...
                                        <path
                                            d="M12.0762 21C16.0942 17.7256 16.1653 24.3574 21.0762 19M3.14233 18.3151C3.15163 17.9381 3.15628 17.7497 3.20185 17.5726C3.24225 17.4156 3.30624 17.2658 3.39161 17.1282C3.48788 16.973 3.62061 16.8396 3.88607 16.573L16.9388 3.46297C17.4624 2.93708 18.2802 2.84896 18.9029 3.25131C19.6412 3.7283 20.2719 4.3551 20.7547 5.09146L20.7885 5.14307C20.813 5.18037 20.8252 5.19902 20.8356 5.21582C21.2189 5.83327 21.1378 6.63294 20.6384 7.16005C20.6248 7.17439 20.6091 7.19018 20.5776 7.22177L7.60428 20.2521C7.32891 20.5287 7.19122 20.6669 7.03052 20.7658C6.88805 20.8534 6.73271 20.9178 6.57023 20.9568C6.38696 21.0008 6.19225 21.0005 5.80282 20.9999L3.07617 20.9955L3.14233 18.3151Z"
                                            stroke="white" stroke-width="2" stroke-linecap="round"
                                            stroke-linejoin="round" />
                                    </svg>
                                </div>
                                <div class="mt-2 mb-2 font-semibold text-black text-xl">¿Aplicar los cambios realizados?
                                </div>
                            </div>

                            <div class="monserrat flex flex-col font-medium text-[#170033] text-center text-sm">
                                <div class="mb-4">Estás por actualizar los datos de esta tienda.</div>
                        ...
                        <div class=" monserrat flex justify-around gap-10">
                            <button @click="cancelEditProduct()"
                                class="w-full mt-4 px-4 py-2 text-sm font-semibold text-black">
                                Cancelar
                            </button>

                            <button @click="goToStoresEdited()"
            ...
            <ModalDefault :show="createProduct" @close="createProduct = false">
                <slot>
                    <div class=" monserrat flex mt-10 flex-col items-center justify-center">
                        ...
                        <div class="mt-2 mb-2 font-semibold text-black text-xl">Crear Tienda</div>
                    </div>

                    <div class="monserrat flex flex-col font-medium text-[#170033] text-center text-sm">
                        <div class="mb-4">Estás por crear una nueva tienda.</div>
                        <div>Verifica que toda la información esté correcta antes de confirmar. Podrás editarla más
                            adelante si es necesario.</div>
                    </div>
                </slot>

                <div class=" monserrat flex justify-around gap-10">
                    <button @click="createProduct = false"
                        class="w-full mt-4 px-4 py-2 text-sm font-semibold text-black">
                        Cancelar
                    </button>

                    <button @click="goToStoresCreated()"
                        class="w-full mt-4 px-2 font-medium py-3 bg-[#875EF8] rounded-full text-white text-sm">
                        Crear tienda </button>
                </div>
            </ModalDefault>
        ...
        <StoreForm v-else :is-editing="isEditing" :beingEdited="beingEdited" :storeName="productId"
            @submit="handleSubmit" ref="productFormRef" />
    </div>
</template>
...
const editCondition = ref("Editar tienda")
...
const handleSubmit = async (product) => {
    try {
        ...
        alert(isEditing.value ? 'Error al actualizar la tienda' : 'Error al crear la tienda')
    }
}
...
function cancelEditProduct() {
    saveProduct.value = false,
        ...
        editCondition.value = 'Editar tienda'
}
...
function goToStoresCreated() {
    router.push({ name: 'stores', query: { success: 'created' } })
}

function goToStoresEdited() {
    router.push({ name: 'stores', query: { success: 'edited' } })
}
